...
caracterizando tendência e sazonalidade de forma bem definida: a tendência 
é modelada como um polinômio de grau arbitrário; e a sazonalidade, como 
apresentando comportamento determinístico.  

Esses fatores limitam a capacidade de aproximação da série estimada 
mediante esse método, bem como não oferecem parâmetros mais apurados para 
dar robustez às previsões desenvolvidas com este algoritmo.  
...
seguinte, sobre os _**Resultados**_.  

Ademais, a construção do algoritmo para implementar o 
**método dos mínimos quadrados** foi feita sem recurso a toolboxs
consolidadas, que aplicam o método mediante funções e rotinas pré-definidas.
Existem inúmeras vantagem em desenhar um algoritmo utilizando somente
comandos e funções básicas, uma delas é conhecer os mecanismos internos do
método mais a fundo, o que nos permite avaliar criticamente as limitações
da própria técnica.  
...
os dados plotados acima, no último gráfico. O seu valor é  
